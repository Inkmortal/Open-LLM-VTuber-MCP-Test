#!/usr/bin/env python3
"""
Run VTuber with virtual camera - Proxy version
Sets up a local proxy to forward VTuber connections
"""
import asyncio
import os
import base64
...
<html>
<head>
    <title>Virtual Camera Demo (Proxy)</title>
    <style>
        body { 
...
<body>
    <div class="container">
        <h1>VTuber Virtual Camera (Proxy)</h1>
        <p>Streaming VTuber through virtual camera with local proxy</p>
...
async def main():
    print("=== VTuber Virtual Camera Service (Proxy) ===")
    ...
    async with async_playwright() as p:
        ...
        try:
            # Now load from localhost which will be proxied
                ...
                try:
                    ...
                    if screenshot and len(screenshot) > 0:
                        ...
                        if success:
                            ...
                            if frame_count % 30 == 0:
                                print(f"\rStreaming: {frame_count} frames ({frame_count // fps}s) - Proxy", end='', flush=True)
