#!/usr/bin/env python3
"""
Run VTuber with virtual camera - WebGL-enabled version
Includes software rendering flags for WebGL/Live2D support in containers
"""
import asyncio
import os
import base64
...
# Start a proxy using Python's built-in server
def start_proxy():
...
<html>
<head>
    <title>Virtual Camera Demo (WebGL)</title>
    <style>
        body { 
...
<body>
    <div class="container">
        <h1>VTuber Virtual Camera (WebGL)</h1>
        <p>Streaming VTuber with WebGL/Live2D support</p>
...
async def main():
    print("=== VTuber Virtual Camera Service (WebGL) ===")
    print(f"Windows host IP: {HOST_IP}")
    print("Using SwiftShader for WebGL software rendering")
    ...
    async with async_playwright() as p:
        # Launch VTuber browser with WebGL software rendering flags
        print("\nLaunching VTuber browser with WebGL support...")
        vtuber_browser = await p.chromium.launch(
            headless=False,
            args=[
                '--no-sandbox', 
                '--disable-dev-shm-usage',
                # WebGL software rendering flags - try angle backend
                '--use-gl=angle',             # Use ANGLE (translates to native GL)
                ...
                '--ignore-gpu-blocklist',     # New name for blacklist
                '--disable-gpu-sandbox',      # Disable GPU process sandbox
                '--enable-unsafe-webgpu',     # Enable WebGPU
                '--enable-features=Vulkan',   # Enable Vulkan support
                '--disable-vulkan-surface',   # But disable Vulkan surface (for SwiftShader)
                '--disable-features=VizDisplayCompositor' # Disable Viz compositor
            ]
        )
        ...
        try:
            # Load from localhost proxy
            vtuber_url = 'http://localhost:12393'
            print(f"Loading VTuber from {vtuber_url} (proxied to {HOST_IP}:12393)")
            ...
            print(f"✅ VTuber page loaded with WebGL support")
            
            # Check WebGL status
            webgl_status = await vtuber_page.evaluate('''() => {
            ...
            print(f"WebGL Status: {webgl_status}")
        ...
        # Launch demo browser (also with WebGL flags for consistency)
        ...
        print("\n✅ Starting VTuber streaming with Live2D rendering...")
        print("WebGL is enabled via SwiftShader software renderer")
        frame_count = 0
        error_count = 0
        fps = 30
                ...
                try:
                    ...
                    if screenshot and len(screenshot) > 0:
                        ...
                        if success:
                            ...
                            if frame_count % 30 == 0:
                                print(f"\rStreaming: {frame_count} frames ({frame_count // fps}s) - WebGL Enabled", end='', flush=True)
