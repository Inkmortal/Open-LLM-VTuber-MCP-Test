#!/usr/bin/env python3
"""
Run VTuber with virtual camera - Final version with all fixes
Addresses navigator.mediaDevices timing and screen size issues
"""
import asyncio
import os
import base64
...
async def main():
    print("=== VTuber Virtual Camera Service (Final) ===")
    ...
    async with async_playwright() as p:
        ...
        # Create demo page with navigator checks
        demo_html = '''
<!DOCTYPE html>
<html>
<head>
    <title>Virtual Camera Demo (Final)</title>
    <style>
        body { 
...
<body>
    <div class="container">
        <h1>VTuber Virtual Camera (Final)</h1>
    ...
    <script>
        ...
        // Initialize virtual camera with proper checks
        function initializeVirtualCamera() {
            try {
                // Check if browser APIs are available
                if (!navigator || !navigator.mediaDevices) {
                    log('⚠️  Media devices not available yet, waiting...', true);
                    setTimeout(initializeVirtualCamera, 500);
                    return;
                }
                
                log('Initializing virtual camera...');
                ...
                // Store original getUserMedia if it exists
                if (navigator.mediaDevices.getUserMedia) {
                    ...
                    log('⚠️  getUserMedia not available yet', true);
                }
                ...
                setTimeout(initializeVirtualCamera, 1000);
            }
        }
